...
int start_day(int year){   // day by day
    int day_start;
    int x1, x2, x3;
    x1= (year - 1)/4;
    x2= (year - 1)/100;
...
int number_days_month (int m, bool leap) //number of month
{
    if(m==1) return(31);
    if(m==2) if(leap) return(29); else return(28);
    if(m==3) return(30);
    if(m==4) return(31);
    if(m==5) return(30);
    if(m==6) return(31);
...
void print_month_name (int m){   //print month name
    if(m==1)
    { 
        skip(7);
        cout << "JANUARY"<< "\n";
    ...
    void print_month (int number_days, int &weekday){  //day and weekday
        int day=1;
        while (day <= number_days){
            ...
            if (weekday == 6){
                ...
                else weekday = weekday = weekday+1;
                day = day +1;
    ...
    while(current_month<=12){
     number_days = number_days_month(current_month, leap); //this is not understand
